package com.venkatesh.offlinesurveyapp.image_survey.view
...
import com.venkatesh.offlinesurveyapp.common.isNetworkAvailable
import com.venkatesh.offlinesurveyapp.databinding.ErrorCaseBinding
import com.venkatesh.offlinesurveyapp.databinding.FragmentItemListBinding
import com.venkatesh.offlinesurveyapp.image_survey.adapter.ImageSurveyRecyclerViewAdapter
import com.venkatesh.offlinesurveyapp.image_survey.viewmodel.ImageSurveyViewModel
...
class ImageSurveyFragment : Fragment(),KodeinAware {

    private val TAG = "ImageSurveyFragment"
    override val kodein by kodein()
    private val modelManager: ModelManager by instance()
    private var columnCount = 1
    private val viewModelFactory : ImageSurveyViewModelFactory by  instance()
    private val viewModel by lazy {
        ViewModelProviders.of(this, viewModelFactory).get(ImageSurveyViewModel::class.java)
    }

    private var startIndex = 0
    private var isLoading = false
    private var imageSurveyRecyclerViewAdapter : ImageSurveyRecyclerViewAdapter? = null

    private lateinit var linearLayoutManager: LinearLayoutManager
    private var totalCount:Int = 0
    private var items : MutableList<Item.Data>? = null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        listItemBinding.recyclerView.layoutManager = linearLayoutManager
    ...
    override fun onSaveInstanceState(@NonNull outState: Bundle) {
        ...
        if (imageSurveyRecyclerViewAdapter != null) {
            outState.putInt(getString(R.string.key_recyclerview_position), linearLayoutManager.findLastVisibleItemPosition())
     ...
     * On scroll changes, calculating totalItems and pastItems from [linearLayoutManager]
    ...
    private fun setRecyclerViewScrollListener(){
        listItemBinding.recyclerView.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                ...
                val pastItems = linearLayoutManager.findLastVisibleItemPosition()
     ...
     * [startIndex] is made zero and [imageSurveyRecyclerViewAdapter] is made null
    ...
    private fun swipeRefreshingLayoutListener(){
        listItemBinding.swipeRefreshingLayout.setOnRefreshListener {
            startIndex = 0
            imageSurveyRecyclerViewAdapter = null
            makePaginationApi()
        }
    }
    ...
    private fun getItemsFromViewModel() {
        if (context != null && context!!.isNetworkAvailable()) {
            ...
            val inputStream = activity?.assets?.open("image_survey.json")
            viewModel.getItems("https://jsoneditoronline.org/?id=ac22ae3452654550a9f579ef3b30ec5a", inputStream)?.observe(this, Observer {
                dismissProgressBarViews()
                if (!countingIdlingResource.isIdleNow) {
                    countingIdlingResource.decrement()
                }
                if (it?.status == LiveDataResource.Status.SUCCESS && it.data != null && it.data.data.isNotEmpty()) {
                    totalCount = 100//it.data.totalResults
     ...
     * [imageSurveyRecyclerViewAdapter] is null getting favourite items from [viewModel]
     * or adding items to adapter and notify it.
     */
    private fun setRecyclerViewAdapterWithData(listOfItems: MutableList<Item.Data>) {
        if (imageSurveyRecyclerViewAdapter == null && recyclerView.adapter == null) {
            items = listOfItems
            imageSurveyRecyclerViewAdapter = ImageSurveyRecyclerViewAdapter(
                modelManager, listOfItems)
            recyclerView.adapter = imageSurveyRecyclerViewAdapter
        } else {
            imageSurveyRecyclerViewAdapter?.addAllItems(listOfItems)
            items?.addAll(listOfItems)
            recyclerView.post {
                imageSurveyRecyclerViewAdapter?.notifyDataSetChanged()
            }
    ...
    /**
     * This interface is used to communicate with click listener in [imageSurveyRecyclerViewAdapter]
    ...
    companion object {
        @JvmStatic
        fun newInstance() = ImageSurveyFragment().apply {}
